...
import { OrderStatusLabelMap, type OrderStatusType } from "../../enum/OrderStatus";
    ...
    <Paper elevation={1} sx={{ mb: 3, borderRadius: 2 }}>
        <Box sx={{ p: 2 }}>
            <Stack direction="row" spacing={3}>
                {[1, 2, 3, 4, 5].map((index) => (
                    <Skeleton key={index} variant="rectangular" width={120} height={48} sx={{ borderRadius: 2 }} />
                ))}
            </Stack>
        </Box>
    </Paper>
...
const ManageOrderByUser: React.FC = () => {
    ...
    const handleChangeTab = (event: React.SyntheticEvent, newValue: "All" | OrderStatusType) => {
        setTab(newValue);
    };
    ...
    const getOrderCount = (status: "All" | OrderStatusType) => {
        return status === "All" ? orders.length : orders.filter(order => order.orderStatus === status).length;
    };
            ...
            <Fade in={true} timeout={800}>
                <Paper elevation={1} sx={{ mb: 3, borderRadius: 2 }}>
                    ...
                    >
                        <Tab
                            label={
                                <Badge badgeContent={getOrderCount("All")} color="primary" max={999}>
            ...
            {filteredOrders.length === 0 ? (
                <Fade in={true} timeout={1000}>
                    ...
                    >
                        ...
                        <Typography variant="body2" color="text.secondary">
                            {tab === "All" ? "Bạn chưa có đơn hàng nào." : `Không có đơn hàng nào ở trạng thái ${OrderStatusLabelMap[tab as OrderStatusType]}.`}
                        </Typography>
                    </Paper>
                </Fade>
            ) : (
